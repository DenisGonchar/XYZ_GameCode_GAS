...
#include "GCCombinedSkeletalMeshComponent.generated.h"
...
UCLASS()
class GAMECODE_API UGCCombinedSkeletalMeshComponent : public USkeletalMeshComponent
{
	GENERATED_BODY()
...
public:
    ...
    void RemoveBodyParts(const TArray<EBodyPart>& PartsToRemove);

    const TMap<EBodyPart, FMeshBodyPart>& GetBodyParts() const;

    void SetBodyPartBoneName(EBodyPart BodyPart, FName BoneName);
...
protected:
    ...
    TMap<EBodyPart, FMeshBodyPart> BodyParts;
    ...
    TArray<EBodyPart> CurrentBodyParts;
};
